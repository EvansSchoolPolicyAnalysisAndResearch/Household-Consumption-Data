...
*Title/Purpose 	: This do.file was developed by the Evans School Policy Analysis & Research Group (EPAR) 
				 ...
				 (purchased, own production and gifts) indicators using the Guinea Bissau Enquête Harmonisée sur le Conditions de Vie des Ménages (EHCVM) (2018-19)
...
*Data source
*-----------
*The Guinea Bissau Enquête Harmonisée sur le Conditions de Vie des Ménages Survey was collected by the National Institute for Statistics 
...
*-----------
*This do.file constructs household consumption by source (purchased, own production and gifts) indicators at the crop and household level using the Guinea Bissau EHCVM dataset.
...
*-----------
*Household ID variables				GB_EHCVM_W1_hhids.dta
*Food Consumption by source			GB_EHCVM_W1_food_consumption_value_by_source.dta
...
//set directories
global GB_EHCVM_W1_raw_data 		"$directory/Guinea Bissau/GNB_2018_EHCVM_v02_M_Stata"
global GB_EHCVM_W1_created_data  "//netid.washington.edu/wfs/EvansEPAR/Project/EPAR/Working Files/439 - Household Food Consumption by Source/Food consumption by source/Consumption Data Curation/created_data"
...
********************************************************************************
global GB_EHCVM_W1_exchange_rate 585.91  		// https://data.worldbank.org/indicator/PA.NUS.FCRF?locations=GB
global GB_EHCVM_W1_gdp_ppp_dollar 250.88			// https://data.worldbank.org/indicator/PA.NUS.PPP?locations=GB
global GB_EHCVM_W1_cons_ppp_dollar 235.17			// https://data.worldbank.org/indicator/PA.NUS.PRVT.PP?locations=GB
global GB_EHCVM_W1_inflation 0.008				// inflation rate 2011-2017. Data was collected during 2010-2011. We want to adjust value to 2017 https://data.worldbank.org/indicator/FP.CPI.TOTL?locations=GB
...
global GB_EHCVM_W1_pop_tot 1970457
global GB_EHCVM_W1_pop_rur 1107850
global GB_EHCVM_W1_pop_urb 862607
...
* HOUSEHOLD IDS *
********************************************************************************
use "${GB_EHCVM_W1_raw_data}/ehcvm_ponderations_gnb2018.dta", clear
merge 1:m grappe using "${GB_EHCVM_W1_raw_data}/s01_me_gnb2018.dta", nogen keep (1 3)
ren hhweight weight
ren s01q03c year_birth
replace year_birth=. if year_birth==9999
...
collapse (max) fhh age_hh weight (sum) hh_members adulteq nadultworking nadultworking_female nadultworking_male nchildren nelders, by(grappe menage)

merge 1:m grappe menage using "${GB_EHCVM_W1_raw_data}/s00_me_gnb2018.dta", nogen keep (1 3)
ren s00q01 region 
...
gen ccf_loc = (1 + $GB_EHCVM_W1_inflation) 
lab var ccf_loc "currency conversion factor - 2017 $SHL"
gen ccf_usd = (1 + $GB_EHCVM_W1_inflation)/$GB_EHCVM_W1_exchange_rate 
lab var ccf_usd "currency conversion factor - 2017 $USD"
gen ccf_1ppp = (1 + $GB_EHCVM_W1_inflation)/ $GB_EHCVM_W1_cons_ppp_dollar
lab var ccf_1ppp "currency conversion factor - 2017 $Private Consumption PPP"
gen ccf_2ppp = (1 + $GB_EHCVM_W1_inflation)/ $GB_EHCVM_W1_gdp_ppp_dollar
...
save  "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_hhids.dta", replace
...
*CONSUMPTION
******************************************************************************** 
use "${GB_EHCVM_W1_raw_data}/s07b_me_gnb2018.dta", clear
merge m:1 grappe menage using "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_hhids.dta", nogen keep (1 3)
label list S07BQ01
...
replace crop_category1=	"Maize"	if  item_code==	7
...
replace crop_category1=	"Potato"	if  item_code==	108
replace crop_category1=	"Sweet Potato"	if  item_code==	109
...
replace crop_category1=	"Cassava"	if  item_code==	114
...
replace crop_category1=	"Non-Dairy Beverages"	if  item_code==	143
replace crop_category1=	"Fruits"	if  item_code==	144
replace crop_category1=	"Vegetables"	if  item_code==	145
replace crop_category1=	"Oils, Fats"	if  item_code==	146
...
gen country=1 
save "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_consumption_purchase.dta", replace
...
while `len'!=0 {
	...
	di "`g'"
	use "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_consumption_purchase.dta", clear
	...
	qui save "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_item_prices_`g'.dta", replace
...
merge m:1 region department grappe item_code food_purch_unit using "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_item_prices_grappe.dta", nogen keep(1 3)
...
drop food_purch_unit 
gen food_purch_unit=food_`f'_unit 
merge m:1 region department item_code food_purch_unit using "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_item_prices_department.dta", nogen keep(1 3)
...
drop food_purch_unit 
gen food_purch_unit=food_`f'_unit 
merge m:1 region item_code food_purch_unit using "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_item_prices_region.dta", nogen keep(1 3)
...
drop food_purch_unit 
gen food_purch_unit=food_`f'_unit 
merge m:1 item_code food_purch_unit using "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_item_prices_country.dta", nogen keep(1 3)
...
save "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_consumption_purchase_`f'.dta", replace
restore
}

merge 1:1 grappe menage item_code crop_category1  using "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_consumption_purchase_consu.dta", nogen
merge 1:1 grappe menage item_code crop_category1  using "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_consumption_purchase_prod.dta", nogen
merge 1:1 grappe menage item_code crop_category1  using "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_consumption_purchase_gift.dta", nogen
...
save "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_food_home_consumption_value.dta", replace

*Food away from home
use "${GB_EHCVM_W1_raw_data}/s07a2_me_gnb2018.dta", clear
...
label var food_consu_value "Value of food consumed over the past 7 days"
label var food_purch_value "Value of food purchased over the past 7 days"
save "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_food_away_consumption_value.dta", replace


use "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_food_home_consumption_value.dta", clear
append using "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_food_away_consumption_value.dta"
merge m:1 grappe menage using "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_hhids.dta", nogen keep(1 3)
...
save "${GB_EHCVM_W1_created_data}/GB_EHCVM_W1_food_consumption_value_combined.dta", replace
...
qui gen Country="Guinea-Bissau"
lab var Country "Country name"
qui gen Instrument="Guinea Bissau EHCVM W1"
...
*generate GID_1 code to match codes in the GB shapefile
gen GID_1=""
replace GID_1="GNB.1_1"  if adm1==6
replace GID_1="GNB.2_1"  if adm1==4
replace GID_1="GNB.3_1"  if adm1==7
replace GID_1="GNB.4_1"  if adm1==5
replace GID_1="GNB.5_1"  if adm1==8
replace GID_1="GNB.6_1"  if adm1==9
replace GID_1="GNB.7_1"  if adm1==3
replace GID_1="GNB.8_1"  if adm1==2
replace GID_1="GNB.9_1"  if adm1==1
...
replace conv_lcu_ppp=	0.004290282	if Instrument==	"Guinea Bissau EHCVM W1"
...
compress
save "${final_data}/GB_EHCVM_W1_food_consumption_value_by_source.dta", replace
